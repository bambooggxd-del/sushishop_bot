...
gamepass_stock = 5000  # Changed from 50000 to 5000
group_stock = 10000    # Kept as 10000

# Channel IDs - Updated from user request
MAIN_CHANNEL_ID = 1361044752975532152
SALES_LOG_CHANNEL_ID = 1402993077643120720
CREDIT_CHANNEL_ID = 1475343873684406353  # Updated from request
TRANSCRIPT_CHANNEL_ID = 1475344141419417612  # Updated from request
DELIVERED_CATEGORY_ID = 1475345768037482662  # Updated from request
ARCHIVED_CATEGORY_ID = 1445086228113264650
BUYER_ROLE_ID = 1361555369825927249

# Ticket category IDs - Updated from request
TICKET_GAMEPASS_CATEGORY_ID = 1475342278976606228  # Updated from request
TICKET_GROUP_CATEGORY_ID = 1475342482844942397    # Updated from request
SHOP_OPEN_CLOSE_CHANNEL_ID = 1475342278976606229  # Updated from request
...
def load_stock_values():
    ...
    if stock_data:
        gamepass_stock = stock_data.get("gamepass_stock", 5000)
        group_stock = stock_data.get("group_stock", 10000)
...
class MyBot(commands.Bot):
    def __init__(self):
        super().__init__(command_prefix=".", intents=intents, help_command=None)  # Changed prefix to "."
...
async def help_command(ctx, command_name: str = None):
    ...
    if command_name:
        ...
        embed = discord.Embed(
            title="📚 wforr Bot Help",
            description="ยินดีต้อนรับสู่ wforr Bot! นี่คือคำสั่งทั้งหมดที่คุณสามารถใช้ได้",
...
async def update_channel_name():
    try:
        # Update shop open/close channel (ID from request)
...
async def update_main_channel():
    try:
        ...
        embed = discord.Embed(title="🍣 wforr 🍣 เปิดให้บริการ", color=0xFFFFFF)  # Changed to white
        embed.add_field(
...
async def handle_open_ticket(interaction, category_name, stock_type):
    ...
    try:
        ...
        # Use updated category IDs
        if stock_type == "gamepass":
            category = interaction.guild.get_channel(TICKET_GAMEPASS_CATEGORY_ID)
        else:
            category = interaction.guild.get_channel(TICKET_GROUP_CATEGORY_ID)
        
        if not category:
            await interaction.response.send_message(f"❌ ไม่พบหมวดหมู่", ephemeral=True)
            return
...
class PersonalNoteModal(Modal, title="📝 จดวันที่เข้ากลุ่ม"):
    ...
    async def on_submit(self, i):
        if self.note.value.strip():
            ...
            embed = discord.Embed(title="✅ บันทึกโน้ตเรียบร้อย", color=0xFFFFFF)
...
class GamepassTicketModal(Modal, title="📋 แบบฟอร์มกดเกมพาส"):
    ...
    async def on_submit(self, i):
        ...
        try:
            ...
            embed = discord.Embed(title="📨 รายละเอียดการสั่งซื้อ", color=0xFFFFFF)
...
class GroupTicketModal(Modal, title="📋 แบบฟอร์มสั่งซื้อ Robux Group"):
    ...
    async def on_submit(self, i):
        ...
        try:
            ...
            embed = discord.Embed(title="📨 รายละเอียดคำสั่งซื้อโรบัคกลุ่ม", color=0xFFFFFF)
...
class DeliveryView(View):
    def __init__(self, channel, product_type, robux_amount, price, buyer):
        ...
        deliver_btn = Button(label="ส่งสินค้าแล้ว 🤍", style=discord.ButtonStyle.success, emoji="🤍")  # Changed from ✅ to 🤍
        ...
        async def deliver_cb(i):
            ...
            confirm_btn = Button(label="ยืนยัน", style=discord.ButtonStyle.success, emoji="🤍")  # Changed from ✅ to 🤍
                ...
                try:
                    ...
                    if self.buyer and not anonymous_mode:
                        try:
                            ...
                            dm_embed.set_footer(text="wforr • ขอบคุณที่ใช้บริการ💖")
...
async def open_cmd(ctx):
    ...
    embed = discord.Embed(
        ...
        description="ร้าน wforr เปิดให้บริการแล้ว", 
        color=0x00FF00
    )
...
async def close_cmd(ctx):
    ...
    embed = discord.Embed(
        title="🔴 ปิดร้านแล้ว", 
        description="ร้าน wforr ปิดให้บริการชั่วคราว", 
        color=0xFF0000
    )
    embed.set_footer(text=f"เวลา: {get_thailand_time().strftime('%d/%m/%y %H:%M')}")
...
async def stock(ctx, stock_type=None, amount=None):
    ...
    if not stock_type:
        embed = discord.Embed(title="📊 สต๊อกสินค้า", color=0xFFFFFF)
        ...
        if amount is None:
            embed = discord.Embed(title="🎮 Gamepass Stock", description=f"**{format_number(gamepass_stock)}**", color=0xFFFFFF)
            await ctx.send(embed=embed)
        else:
            try:
        ...
        if amount is None:
            embed = discord.Embed(title="👥 Group Stock", description=f"**{format_number(group_stock)}**", color=0xFFFFFF)
            await ctx.send(embed=embed)
        else:
            try:
...
async def rate(ctx, rate_type=None, low_rate=None, high_rate=None):
    ...
    if rate_type is None:
        embed = discord.Embed(title="📊 เรทโรบัคปัจจุบัน", color=0xFFFFFF)
...
async def qr(ctx):
    ...
    embed = discord.Embed(
        title="⚠️โน๊ตสลิปว่า: เติมโรบัคกับ wforr⚠️ ช่องทางการโอนเงิน", 
        color=0x00CCFF
    )
    embed.add_field(
    ...
    async def copy_cb(i):
        await i.response.send_message(f"```120-239181-3 ⚠️โน๊ตสลิปว่า: เติมโรบัคกับ wforr⚠️```", ephemeral=True)
...
async def update_presence():
    await bot.change_presence(
        activity=discord.GameGame(name="บอทเครื่องคิดเลขของ wforr | .help")
    )
...
async def on_message(message):
    if message.channel.id == CREDIT_CHANNEL_ID:
        if message.author != bot.user:
            await asyncio.sleep(2)
            for emoji in ["🤍", "🍣"]:  # Changed from ❤️ to 🤍
                try:
